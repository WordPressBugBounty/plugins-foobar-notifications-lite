...
function foobar_enqueue_polyfills() {
    ...
    $deps = apply_filters( 'foobar_polyfills_deps', array() );
    wp_enqueue_script(
        $handle,
        $src,
        $deps,
...
function foobar_admin_freetrial_url() {
    return foobar_fs()->get_trial_url();
}

/**
 * Returns the purchase URL for the PRO version
 */
function foobar_admin_purchase_url() {
    return foobar_fs()->get_upgrade_url();
}
